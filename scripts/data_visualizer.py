...
def plot_pie(df, column, limit=None):
...
def multi_hist(sr, rows, cols, title_text, subplot_titles):
...
def plot_scatter(df, x_col, y_col, marker_size, hover=[]):
    fig = px.scatter(
            df,
            x=x_col,
            y=y_col,
...
def plotly_plot_hist(df:pd.DataFrame, column:str, color:str='cornflowerblue')->None:
